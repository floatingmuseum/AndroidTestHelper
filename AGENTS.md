...
- Current behavior:
  ...
  - Display a non-hideable line-number column before the seven content columns, using the original 1-based file line numbers even after filtering. Fit its width to the measured total file line count or header, whichever is wider, plus cell padding; do not impose a fixed minimum width. Left-align the header and numbers, keeping numbers on a single line. Line numbers, content cells and line-number measurement must share the same complete body text style. Clicking a content header hides that column; restore controls must remain accessible even when every content column is hidden. Long cells wrap; headers and rows share widths and horizontal scrolling.
