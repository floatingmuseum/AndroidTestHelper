...
- Current behavior:
  ...
  - The Log module opens the file viewer in an independent, resizable desktop window, maximized by default. Mount it at App scope so switching modules does not close it; window setup lives in `LogFileViewerWindow.jvm.kt`. It is independent of device selection and capture state. Capture results offer View actions for both full and filtered files; path links still reveal files in the OS file manager.
  - `LogFileViewer.kt` owns seven-column parsing; `LogFileViewerController` owns temporary column visibility, search, window lifecycle and a bounded cache of visible text. Keep local file IO and the picker in `LogFileReader.jvm.kt`.
  - Display a non-hideable line-number column before the seven content columns, using the original 1-based file line numbers even after filtering. Fit its width to the measured total file line count or header, whichever is wider, plus cell padding and space for the inline hover pencil; do not impose a fixed minimum width. Left-align the header and numbers, keeping numbers on a single line. Line numbers, content cells and line-number measurement must share the same complete body text style. Clicking a content header hides that column; restore controls must remain accessible even when every content column is hidden. Long cells wrap; headers and rows share widths and horizontal scrolling.
  - Viewer search uses the same literal OR / case rules as capture filters, includes hidden columns, and scans the file on the IO dispatcher. Index the whole result set using byte offsets, lengths and original line numbers, preserving UTF-8 / CRLF boundaries. Show every indexed row in one LazyColumn without pagination; load visible text asynchronously in 128-row blocks with at most 8 cached blocks and 2 concurrent reads. Do not load all log strings into memory or perform file IO in composition.
  - Preserve headers, separators, stack traces and unrecognized lines in the message column. Viewing never rewrites a log file. Cancel stale reads on file/query changes and release the index and cached rows on close; opening a file resets search and hidden columns.
  - The viewer has a horizontally collapsible right-hand notes timeline, ordered by original line number. When collapsed, keep only clickable note line numbers and an expand control. Expanded cards show the line number and a right-aligned timestamp on one row, with edit/delete in a vertical-three-dot menu beside the timestamp, then the log preview above the note text. Each log row reveals an inline pencil beside its line number only on hover; never add a separate pencil column or shift column widths on hover. The pencil opens an editor with a read-only full source snapshot and editable note text without changing the timeline's expansion state. Keep one editable note per original line, and confirm deletion or discarding unsaved edits. Column restore buttons must use natural height and wrap to additional rows when needed so every label and action remains visible.
  - `LogFileNotesController` owns notes loading, editing and save state; `LogFileNotesRepository.jvm.kt` performs sidecar IO on the IO dispatcher. Persist versioned UTF-8 JSON to `<logPath>.notes.json` beside the user-selected log, automatically restoring it on open. Never rewrite the log. Replace sidecars through a completed temporary write; preserve drafts on failure, reject invalid/unsupported sidecars, and check the loaded revision before overwriting externally changed notes. Prevent file/window switching while a save is in flight; stale loads must not populate a different file.
  - Annotated log rows retain a background color regardless of selection. Hovered rows use a distinct background and return to their persistent note/normal color on pointer exit. Clicking a note in either timeline mode locates its original line through the index, validates its saved raw text, and scrolls to the row. Once its text is available, pulse the target background twice and return to the persistent note color; repeated clicks restart the pulse. Clear search only if it hides the target, restore content columns if all are hidden, and retain the timeline's collapsed state during navigation. A missing/changed source must report that it cannot be located reliably while retaining the saved snapshot. Cancel stale jumps and pulses on file/query changes. Notes must not expand the bounded cache into a full-file text load.
