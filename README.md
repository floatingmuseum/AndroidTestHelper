...
- **日志模块**
  - 项目默认命令使用 `logcat -b all -v threadtime -v year *:V` 连续采集，按 Android Studio 风格输出日期、毫秒时间、PID-TID、Tag、进程名、等级和消息；始终保留，不能修改或删除。
  - “自定义命令”中展示项目默认命令；新增时在单行输入框预填 `adb -s <serialNumber> shell logcat`，可自行删改并命名保存，也可编辑、删除已保存的命令。有自定义命令时，日志页通过下拉菜单切换，第一项始终是“项目默认命令”；命令与当前选择保存在 `AndroidTestHelperData/cache/log_commands.json`。
  - 自定义输入支持完整 `adb [-s <serialNumber>] [shell] logcat ...` 或 `logcat ...`，执行时自动使用当前选中的设备；支持引号参数，不支持管道、重定向、清空日志、二进制输出或写入设备文件。自定义抓取保留原始文本格式，仍可使用关键字过滤；`-d` / `-t` 等主动结束的抓取正常完成后显示“已完成”。
  - 进程名取自设备当前进程列表，每 5 秒刷新；不可用时显示 `-`，历史日志的 PID 对应名称可能与当前不同。
  - 支持普通文本关键字过滤，多个关键字用 `|` 分隔并匹配任意一个，可切换大小写敏感；不解析正则或命令参数。
  - 有关键字时同时保存完整日志和 `_filtered.log`，留空只保存完整日志。抓取期间固定本次条件，停止后可分别点击路径定位文件。
  - 开始采集或点击“保存关键字”会保存整组过滤条件及大小写选项；历史去重、最近优先，最多保留 50 条，可复用或删除，重启后仍保留。
  - 日志保存到应用运行目录下的 `AndroidTestHelperData/logs`。
  - 抓取中显示悬浮停止按钮，区分完成、用户停止和设备中断状态。
  - “打开日志查看器”会打开独立窗口，默认最大化，可调整大小；也可从最近抓取结果直接查看完整或过滤日志，无需连接设备，不占用主界面的设备和命令面板。
  - 查看器最左侧显示从 1 开始的原文件行号，搜索过滤后保留原始行号；行号表头和数字左对齐，列宽按最大行号或表头文字自适应。其后按日期、时间、PID-TID、Tag、进程、等级、消息显示七个内容列。点击内容列表头临时隐藏该列，上方可恢复单列或全部列，行号列不可隐藏，长内容自动换行。
  - 日志消息与等级标记参考 Android Studio Logcat 配色：V 灰、D 青、I 绿、W 黄、E 红，A/F 使用红色消息和深红等级底色。等级底色仅用于字母标记，保留整行的备注、悬停和定位高亮；搜索命中仍优先显示匹配高亮。
  - 搜索覆盖整个文件及隐藏列，支持 `|` 分隔的普通文本关键字、大小写选项和匹配高亮；全部日志或匹配结果在一个列表里连续滚动，无需翻页，可重新读取，不修改原文件。
  - 后台建立整个文件的行索引，只按需读取和绘制可见行，并限制正文缓存；大文件不会一次性加载全部文本。关闭查看器会取消读取并释放索引和缓存。
  - 鼠标悬停日志行时，行号旁显示笔形按钮，点击后可添加或编辑备注，编辑框保留完整日志原文。笔形按钮位于行号单元格内，不设独立列，悬停不改变列宽。隐藏列的恢复按钮自适应高度，空间不足时换行，确保文字和所有按钮完整可见。
  - 右侧备注时间轴按原始行号排列，行号和靠右的时间戳位于同一行，时间旁的竖向三点菜单提供编辑和删除操作；下方先显示日志摘要，再显示备注内容。侧栏支持水平展开和收起；收起后仅显示可点击的备注行号，仍可跳转。通过日志行的笔形按钮添加或编辑备注时保持侧栏的展开或收起状态，删除备注需要确认。
  - 已备注的日志行持续显示背景色，鼠标悬停时使用另一种底色，移开后恢复备注底色。点击备注会跳转至原始行，背景短暂闪烁两次后恢复备注底色；目标被搜索隐藏时自动清空搜索。定位前核对原文，日志行被删除或修改时会提示无法准确定位，并保留备注中的日志快照。
  - 备注保存到日志旁的 `<日志文件名>.notes.json`，重新打开同一日志时自动恢复，不改写日志。移动日志时请一同移动备注文件；重命名日志时同步修改备注文件名前缀。保存失败保留输入，放弃未保存修改需要确认；损坏或被外部修改的备注文件不会被静默覆盖。
  - 支持本工具保存的七列格式和原始 `threadtime` 格式；文件头、分隔线、堆栈及未知格式保留在消息列。查看状态不写入设置，重新打开文件会重置隐藏列和搜索。
- **设置模块**
  - 控制命令时间戳显示。
  - 控制命令耗时显示。
  - 配置文件管理默认根目录。
